...
void MigratorThread::MigrateListsDB() {
  ...
  while (!fin) {
    ...
    for (auto k : keys) {
      ...
      blackwidow::Status s = db->LRange(k, pos, pos + g_pika_conf->sync_batch_num() - 1, &list);
      if (!s.ok()) {
      ...
      while (s.ok() && !should_exit_ && !list.empty()) {
        ...
        list.clear();
        s = db->LRange(k, pos, pos + g_pika_conf->sync_batch_num() - 1, &list);
        if (!s.ok()) {
          LOG(WARNING) << "db->LRange(key:" << k << ", pos:" << pos
            << ", batch size:" << g_pika_conf->sync_batch_num() << ") = " << s.ToString();
        }
